...
- `argocd-server-network-policy` admits the API/UI port (8080) and the metrics port (8083) from the `argocd` namespace only, plus the metrics port (8083) from `monitoring` so Prometheus can scrape argocd-server. No other ingress, and 8080 is never reachable from `monitoring`. Generated by `bootstrap/build-argocd.sh`.
